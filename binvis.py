...
def main(hexargs,curvecolor):
    ...
    #hexargs.append("-c gradient")
    options, args = parser.parse_args(hexargs)
    if len(args) not in [1, 2]:
        parser.error("Please specify input and output file.")
    try:
        d = open(args[0],encoding="latin-1").read()
    except FileNotFoundError:
        print("File could not be found to create hex visulization!")
        return ""
    ...
    elif curvecolor == "hilbert":
        csource = ColorHilbert(d, block)
    elif curvecolor == "gradient":
